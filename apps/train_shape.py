    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            if train_idx % opt.freq_plot == 0:
                print(
                    'Name: {0} | Epoch: {1} | {2}/{3} | Err: {4:.06f} | Err_SR: {5:.06f} | Err_mlp1: {6:.06f} | Err_mlp2: {7:.06f} | LR: {8:.06f} | Sigma: {9:.02f} | dataT: {10:.05f} | netT: {11:.05f} | ETA: {12:02d}:{13:02d}'.format(
                        opt.name, epoch, train_idx, len(train_data_loader), error.item(), error_SR.item(), error_mlp1.item(), error_mlp2.item(), lr, opt.sigma,
