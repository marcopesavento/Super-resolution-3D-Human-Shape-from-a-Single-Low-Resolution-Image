...
class HGPIFuNet(BasePIFuNet):
    ...
    def query_lr(self, points, calibs, transforms=None, labels_lr=None):
        ...
        if labels_lr is not None:
            ...
            newlabels=in_bb * labels[:,i])
            with torch.no_grad():
                    ws=in_bb.size(2) / in_bb.view(in_bb.size(0),-1).sum(1))
                    gammas=1 - newlabels[-1].view(newlabels[-1].size(0),-1).sum(1) / in_bb.view(in_bb.size(0),-1).sum(1))
        ...
        for im_feat in range(0,len(self.im_feat_list_lr)):
            ...
            self.point_local_feat=torch.cat(point_local_feat_list, 1)
            print(self.point_local_feat.shape)

            # out of image plane is always set to 0
            pred = self.in_img[:,None].float() * self.mlp_lr(self.point_local_feat) #non so se va lo [0]!!!!!!
    ...
    def query_hr(self, transforms=None, labels_hr=None):
        ...
        for im_feat in range(0,len(self.im_feat_list_lr)):
            ...
            point_local_feat_list = [self.point_local_feat,self.intermediate_preds_list_lr[im_feat]]
    ...
    def get_error_lr(self):
        ...
        for preds in self.intermediate_preds_list_lr:
            error += CustomBCELoss(preds, self.labels_lr, self.gamma, self.ws) #cambio qua
    ...
    def get_error_hr(self):
        ...
        for preds in self.intermediate_preds_list_hr:
            error += CustomBCELoss(preds, self.labels_hr, self.gamma, self.ws)
        error /= len(self.intermediate_preds_list_hr)
        
        return error
    
    def get_errorSR(self,images_hr):
        '''
        Hourglass has its own intermediate supervision scheme
        '''
    ...
    def forward(self, images_lr, images_hr, points, calibs, transforms=None, labels_lr=None, labels_hr=None):
        ...
        error_SR = self.get_errorSR(images_hr)

        error=self.opt.mlp1*error_mlp1+self.opt.mlp2*error_mlp2+self.opt.srweight*error_SR

        return res, error, error_mlp1, error_mlp2, error_SR